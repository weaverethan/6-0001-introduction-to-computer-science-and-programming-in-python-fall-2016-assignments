total_cost = 300000
portion_down_payment = 156024 #total_cost*.66
current_savings = 0
annual_salary = 67710
monthly_salary = annual_salary/12
portion_saved=.743 * monthly_salary
annual_return = .04
monthly_return = annual_return/12
monthcounter = 0
yearcounter = 0
annual_raise = .03



while current_savings < portion_down_payment:
    current_savings = round((portion_saved + current_savings)+(monthly_return*(portion_saved + current_savings)))
    monthcounter = monthcounter+1
    if monthcounter%12 == 0:
        yearcounter = yearcounter+1
        annual_salary=annual_salary*annual_raise+annual_salary
        print("RAISE! $"+str(annual_salary))
    print("month "+ str(monthcounter) +" savings: $"+str(current_savings) )

print("it took "+ str(monthcounter) +" months to save $"+str(current_savings))
print("your salary is $"+str(annual_salary))

